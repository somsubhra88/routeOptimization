def point(addr1, addr2, pincode, city, state):
    ...
    geoCodeAPI = "http://fms-bulk-sage-app.vip.nm.flipkart.com:8888/geocode?"
...
def distance(point1, point2):
    ...
    # print(url2)
    try:
        resp = requests.get(url2, headers=header, timeout=5)
        ...
        while notFound:
            ...
            url2 = 'https://maps.flipkart.com/api/v1/directions?point=' + \
                   ...
                   '&point=' + str(point2['lat']) + ',' + str(point2['lng']) + \
                   "&key=" + key
            resp = requests.get(url2, headers=header, timeout=15)
            if resp.json()['info'].get('errors') is None:
                notFound = False
                time = json.loads(resp.text)['paths'][0]['time'] / 3600000
            else:
